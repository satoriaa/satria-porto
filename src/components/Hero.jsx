...
const Hero = ({ retroShadow, btnClass, scrollToSection }) => {
  const { language } = useLanguage();
  const t = translations[language].hero;
  
  const roles = [t.role1, t.role2];
  ...
  return (
    <section id="home" className="stack-section flex flex-col justify-center items-center py-20 px-4 sm:px-6 bg-[#dfdfdf] border-b-8 border-black overflow-hidden relative" style={{ "--stack-i": 0 }}>
      <div className="absolute inset-0 opacity-[0.03] z-0" style={{ backgroundImage: 'linear-gradient(#000 1px, transparent 1px), linear-gradient(90deg, #000 1px, transparent 1px)', backgroundSize: '50px 50px' }} />
      ...
      <div className="absolute inset-0 z-0 pointer-events-none">
        ...
        <div className="absolute top-1/2 right-1/4 w-2 h-10 bg-black animate-spin opacity-[0.05]" style={{ animationDuration: '8s' }}></div>
      </div>

      <div className="absolute -top-20 -left-20 w-[400px] h-[400px] bg-blue-600 rounded-full blur-[150px] opacity-[0.06] z-0" />
      <div className="absolute -bottom-10 -right-10 font-black text-[12rem] text-black/[0.015] select-none pointer-events-none hidden md:block uppercase tracking-tighter italic animate-pulse">SATORIAA</div>
      ...
      <div className="grid md:grid-cols-12 gap-4 sm:gap-6 md:gap-10 max-w-6xl w-full items-stretch relative z-10">
        ...
        <div className="md:col-span-7 border-4 border-black p-4 sm:p-6 md:p-10 bg-white flex flex-col justify-center relative shadow-[16px_16px_0px_0px_rgba(0,0,0,1)] hover:shadow-[8px_8px_0px_0px_rgba(30,58,138,1)] transition-all duration-300 min-h-0 md:min-h-[500px] group/card">
          <div className="mb-4 sm:mb-6">
            <div className="flex items-center gap-2 mb-1">
              ...
              <p className="font-black text-black tracking-widest uppercase text-[10px] group-hover/card:text-blue-700 transition-colors">{t.systemActive}</p>
            </div>
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-black uppercase italic tracking-tighter text-blue-800 relative inline-block">
              {t.nameExe}
              <span className="absolute top-0 left-0 w-full h-full text-red-500 opacity-0 group-hover/card:opacity-30 group-hover/card:animate-ping pointer-events-none text-2xl sm:text-3xl md:text-4xl font-black uppercase italic tracking-tighter">{t.nameExe}</span>
            </h2>
          ...
          <p className="text-base sm:text-lg md:text-xl font-bold border-l-8 border-blue-700 pl-4 sm:pl-6 mb-6 sm:mb-8 leading-relaxed text-black max-w-lg mt-0">
            {t.description}{' '}
            <span className="underline decoration-yellow-400 decoration-4 hover:bg-yellow-400 transition-colors">{t.description2}</span>{' '}
            {t.description3}
          </p>

          <div className="flex flex-wrap gap-3 sm:gap-4">
            <a href="#projects" onClick={(e) => scrollToSection(e, 'projects')} className={`${btnClass} text-base sm:text-lg px-6 sm:px-8 py-3 bg-black text-white hover:bg-blue-700 hover:-translate-y-1 hover:shadow-[8px_8px_0px_0px_rgba(59,130,246,0.5)] transition-all flex items-center gap-3`}>
              <span>⚙️</span> {t.btnWorks}
            </a>
            <a href="/cv.pdf" download className={`${btnClass} text-base sm:text-lg px-6 sm:px-8 py-3 bg-blue-700 text-white hover:bg-black hover:-translate-y-1 hover:shadow-[8px_8px_0px_0px_rgba(255,255,255,0.5)] transition-all flex items-center gap-3`}>
              <span>📄</span> {t.btnCV}
            </a>
          </div>
        </div>
        ...
        <div className="md:col-span-5 flex flex-col h-full relative z-10 group/img">
          <div className="bg-gradient-to-r from-[#000080] to-[#1084d0] text-white text-[11px] px-3 py-1.5 flex justify-between items-center font-bold mb-1 border-b-2 border-black relative z-20">
            <span className="group-hover/img:animate-pulse">{t.identityCheck}</span>
          ...
          <div className="w-full flex-1 border-2 border-black overflow-hidden bg-black relative cursor-crosshair flex flex-col">
            <div className="w-full h-full min-h-[300px] md:min-h-0 flex-1 overflow-hidden">
              <img src={profilSatria} alt="Satria Rizky" className="w-full h-full object-cover grayscale group-hover/img:grayscale-0 group-hover/img:scale-110 transition-all duration-1000" />
            </div>
          ...
          <div className="absolute -bottom-5 -left-5 bg-yellow-400 border-4 border-black px-4 py-2 font-black text-xs uppercase shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] rotate-[-5deg] z-30 group-hover/img:rotate-0 transition-transform duration-300">{t.sticker}</div>
        </div>
